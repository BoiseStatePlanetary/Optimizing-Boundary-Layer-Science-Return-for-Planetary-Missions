...
Within planetary mission concepts development, there is a focus on enabling increased science return via improved usage of limited resources, such as power and data volume, especially for those using a small spacecraft.  
Such resource optimization is additionally important for concepts with the aim of returning detailed observations of transient, active processes \cite{MCESAG}; such concepts will require observation- and downlink-prioritization schemes that can strategically identify when the activity of interest occurs so that a smaller number of observations or measurements can still yield high-value science results.
...
In particular, between the large dust storms, dust is continually lofted due to winds or dust devils (\textcolor{red}{Figure \ref{fig:dust_lofting_diagram}}), but the rates of such lifting phenomena are unconstrained and thus there remain uncertainties in the total dust lofting budget and variability of the airborne dust distribution. 
%Brian had some stuff on the level of uncertainty - see 10th Mars poster

\begin{figure}
    \label{fig:dust_lofting_diagram}
    \centering
    \includegraphics[width=0.45\textwidth]{example-image.pdf}
    \caption{Dust lofting diagram/overview. \textcolor{red}{Not fully fleshed out idea, but likely want a figure that illustrates how dust can be lifted into the atmosphere and highlighting why we wish to detect dust devils early. Could also show a time series pressure trace with the FWHM window highlighted to indicate when important science is occurring}}
\end{figure}
...
One option is to build an adaptive sampling strategy that is responsive to the real-time environment; i.e., focus high-power/high-data volume observations on times when events of interest are occurring. Such a strategy has been discussed in the planetary community \citep[e.g.,][]{wagstaff_ntrs_2009,Diniega2022,Jackson2023} but is untested, and questions remain about what technical capabilities and \textit{a priori} understanding of the environment are needed to ensure high science return and resource conservation. 
This project answers these questions with a proof-of-concept study simulating real-time martian dust devil detection and measurement. 
...
\subsection{Case Study: Low-Resource Detection of Dust Devils}
Past work has developed autonomous methods for detecting dust devils within already acquired data - time-series analysis of pressure signals and signatures within visible images \citep[e.g.][]{Castano2008,Guzewich2023}. 
Identification of these events, if done onboard the spacecraft, could be used for downlink prioritization which enables better use of a small data volume envelope \citep{wagstaff_ntrs_2009}. 

As described above, our work instead focuses on optimizing data collection, using a low-resource instrument to monitor for the activity and, when detected, ``triggering'' a shift to a high-resource, alert observation mode. 
...
For our study, we used the Mars 2020 rover's \gls{MEDA} dataset. 
\acrshort{MEDA} collects data using a number of meteorological instruments including a pressure sensor and a radiation and dust sensor. 
Its data is useful for studying a number of processes related to both short- and long-term events of interest, including convective vortices. 
Prior studies have mined this full science dataset to find dust devils and estimate their contributions to the martian dust budget \citep[e.g.,][]{Jackson2022,Jackson2022b}.
...
Our simulated ``collected observations'' will be sampled from the MEDA dataset: we include the high-resource dataset (over a specified duration) only \emph{after} a detection is generated based on the low-resource observations and the spacecraft's observation mode moves from the ``monitoring mode'' to an ``alert mode.''

For our monitoring instrument, we use the \gls{MEDA} pressure sensor (PS) time-series observations; this dataset has been used for dust devil detection via a simple threshold in pressure variations \citep{Jackson2022}. 
...
For another comparison, we define a ``low-resource'' observation mode and instrument suite, which includes all of the \gls{MEDA} instruments \emph{except} for the wind sensors (listed in Table \ref{tab:PayloadModes}) along with the monitoring PS; this sampling provides an observational dataset that is more comparable with the science achieved with the full suite of \gls{MEDA} meteorological time-series and we avoid the long-heating/temperature stabilization time needed for the relative humidity sensor \cite{}.
For the ''high-resource'' instrument suite used during the alert observation mode, we include the \gls{MEDA} wind sensors (WS) and a navigation camera (NavCam) to observe the dust devil structure and its approach relative to the sensor suite.
...
This project aims to quantify how much science autonomy algorithms can increase the ratio of science-returned to resources, using a demonstration relevant to small in-situ spacecraft with a relatively light power and data budget. 
Our results will yield engineering and operational guidance for deploying data science algorithms onboard small spacecraft or Earth field instrumentation, and better enable NASA to invest in technology addressing likely future community-identified needs.

The aim for this case study is to strategically collect detailed, high frequency observations during a dust devil that are needed to improve estimates of dust lofting, which remains highly uncertain.
In our studies, we will vary (1) the method of initial dust devil detection and (2) the amount and type of data that is collected during a specified window following detection.
Three algorithms will be evaluated for their ability to detect the early signs of a dust devil and trigger collection of useful high frequency measurements, as previous work suggests is possible (i.e., a about 1\% pressure drop over ~10s indicates a dust devil \cite{8inabstract}, Figure \ref{}). 
Our first detection scheme will use a simple pressure change threshold, defined based on an \textit{a priori} understanding of Martian dust devils (Figure \ref{}); this is the current accepted method for identifying dust devils \cite{8inabstract}. 
Our second scheme will involve an expert system (e.g., using time-series statistics and probabilistic, tunable thresholds). 
Our third scheme will train a \acrlong{ML} (e.g., a \acrfull{LSTM}) model to detect incoming dust devils in pressure time-series data.
Our fourth and final scheme evalues an \acrshort{ML}-based transformer model in the same manner as the \acrshort{LSTM} model.
